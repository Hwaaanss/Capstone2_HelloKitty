...
def load_test_store_data(store_code):
    ...
    try:
        ...
        # 학습에 사용된 동일한 데이터 파일 사용
        original_path = "./dataset/BasicData_Prep3.csv"
...
def get_available_test_codes():
    ...
    try:
        ...
        # 학습에 사용된 동일한 데이터 파일 사용
        original_path = "./dataset/BasicData_Prep2.csv"
        if not os.path.exists(original_path):
            return []
                ...
                with st.spinner("점포 정보를 분석하고 예측하는 중..."):
                    ...
                    if store_data and st.session_state.model:
                        ...
                        if prediction is not None:
                            ...
                            # 202412 → 202501 미래 예측 강조
                            info_msg = f"🔮 **실전 미래 예측**\n\n"
                            info_msg += f"📅 입력: {current_month}월 데이터\n"
                            info_msg += f"📅 예측: {predict_month}월 (미래)\n\n"
                            info_msg += f"**AI 예측값: {prediction:.2f}**\n\n"
                            info_msg += f"💡 {predict_month}월은 아직 오지 않은 미래 시점으로, 이 예측값은 실제 미래 성과를 예상한 것입니다."
