...
from health_score_calculator import StoreHealthCalculator
...
def initialize_session_state():
    ...
    if 'health_calculator' not in st.session_state:
        st.session_state.health_calculator = StoreHealthCalculator()
...
def create_health_gauge_chart(health_score, grade, emoji):
    """
    건강도 게이지 차트 생성
        ...
        if st.session_state.chat_history:
            ...
            for i, chat in enumerate(reversed(st.session_state.chat_history)):
                with st.container():
                    ...
                    # 건강도 게이지 차트 표시 (답변 아래)
                    if st.session_state.store_data:
                        analysis = st.session_state.health_calculator.analyze_store_status(
                            st.session_state.store_data
                        )

                        # 게이지 차트 표시
                        fig = create_health_gauge_chart(
                        ...
                        if analysis['key_metrics']:
                            ...
                            for idx, (key, value) in enumerate(analysis['key_metrics'].items()):
                                with cols[idx]:
