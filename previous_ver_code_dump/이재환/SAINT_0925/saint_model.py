...
class SAINTWrapper:
    ...
    def create_data_loaders(self, X_train, y_train, X_val, y_val, X_test, y_test):
        ...
        try:
            ...
            trainloader = DataLoader(train_ds, batch_size=self.config.model.batch_size, shuffle=True, num_workers=0)

            valid_ds = DataSetCatCon(X_val_dict, y_val_dict, self.cat_idxs, dtask, self.continuous_mean_std)
            validloader = DataLoader(valid_ds, batch_size=self.config.model.batch_size, shuffle=False, num_workers=0)

            test_ds = DataSetCatCon(X_test_dict, y_test_dict, self.cat_idxs, dtask, self.continuous_mean_std)
            testloader = DataLoader(test_ds, batch_size=self.config.model.batch_size, shuffle=False, num_workers=0)
