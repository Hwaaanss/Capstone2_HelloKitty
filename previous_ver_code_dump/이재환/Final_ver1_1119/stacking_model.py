...
class StackingEnsembleWrapper:
    def __init__(self, config: Config):
        ...
        self.base_model_metrics = {}  # 각 베이스 모델의 성능 기록
    ...
    def _prepare_data_by_store(self, df, target_columns, test_size=0.2, val_size=0.1):
        """ENCODED_MCT 기준으로 데이터셋을 분리 (시간적 분리 적용)"""
        ...
        if 'TA_YM' in df.columns:
            # ===== 시간적 분리: Test는 가장 최근 월만, Train/Val은 그 이전만 =====
            # 각 점포별 최대 월을 구한 뒤, 전체에서 가장 많이 나타나는 월을 Test 기준으로 설정
            max_ta_ym_per_store = df.groupby('ENCODED_MCT')['TA_YM'].max()

            # 가장 흔한 최대 월 (대부분 점포가 가진 마지막 월)
            test_cutoff_month = max_ta_ym_per_store.mode().values[0]

            print(f"Time-based split: Test uses TA_YM={test_cutoff_month} only")
            print(f"                  Train/Val use TA_YM < {test_cutoff_month}")

            # Test set: test_cutoff_month인 데이터만 (모든 점포에서)
            test_mask = df['TA_YM'] == test_cutoff_month
            X_test = X[test_mask].copy()
            y_test = y[test_mask]

            # Train/Val: test_cutoff_month보다 이전 데이터만
            train_val_mask = df['TA_YM'] < test_cutoff_month
            X_remaining = X[train_val_mask].copy()
            y_remaining = y[train_val_mask]

            print(f"Test set: {X_test.shape} (TA_YM={test_cutoff_month})")
            print(f"Train+Val set: {X_remaining.shape} (TA_YM < {test_cutoff_month})")
            ...
            print(f"Train set: {X_train.shape} ({len(train_stores)} stores, TA_YM < {test_cutoff_month})")
            print(f"Validation set: {X_val.shape} ({len(val_stores)} stores, TA_YM < {test_cutoff_month})")
    ...
    def train_model(self, X_train, y_train, X_val, y_val):
        ...
        for name, model in self.base_models:
            ...
            if self.config.model.task == 'regression':
                ...
                self.base_model_metrics[name] = {
                    'mse': val_mse,
                    'rmse': val_rmse,
                    'r2': val_r2
                }
                print(f"  ✓ {name} training completed - RMSE: {val_rmse:.4f}, R2: {val_r2:.4f}")
            else:
                val_acc = accuracy_score(y_val, val_pred)
                self.base_model_metrics[name] = {
                    'accuracy': val_acc
                }
                print(f"  ✓ {name} training completed - Accuracy: {val_acc:.4f}")
    ...
    def get_feature_importance(self, X):
        """가장 성능이 좋은 베이스 모델의 feature importance 반환"""
        if not self.base_models or not self.base_model_metrics:
            return None

        # 최고 성능 모델 찾기
        if self.config.model.task == 'regression':
            best_model_name = min(self.base_model_metrics.items(),
                                 key=lambda x: x[1]['rmse'])[0]
        else:
            best_model_name = max(self.base_model_metrics.items(),
        ...
        try:
            ...
            print(f"\nFeature importance extracted from best base model: {best_model_name.upper()}")
            return importance
    ...
    def save_model(self, save_dir="./models"):
        ...
        try:
            ...
            metadata = {
                ...
                'base_model_metrics': self.base_model_metrics,
    ...
    def load_model(self, save_dir="./models"):
        ...
        try:
            ...
            self.base_model_metrics = metadata.get('base_model_metrics', {})
