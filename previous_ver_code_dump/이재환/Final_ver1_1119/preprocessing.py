...
def run_preprocessing(raw_data_dir="./dataset/raw", output_dir="./dataset"):
    ...
    # 10. quaters 컬럼 생성 및 계절성 피처 추가
    print("\n[Step 10] Creating quarters and seasonality features...")
    ...
    # 월 정보 추출 (임시로 사용, 이후 제거)
    month = data_prep['TA_YM'] % 100

    # 주기성 반영 (sin/cos 변환으로 월의 순환 특성 포착)
    data_prep['month_sin'] = np.sin(2 * np.pi * month / 12)
    data_prep['month_cos'] = np.cos(2 * np.pi * month / 12)
    print(f"  - Added cyclical features (month_sin, month_cos)")
