...
class OptunaXGBoostTuner:
    ...
    def tune(self, n_trials=100, timeout=None, show_progress_bar=True):
        ...
        self.study = optuna.create_study(
            direction=direction,
            study_name=f'xgboost_{self.task}_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Optuna XGBoost Hyperparameter Tuning')
