...
from utils import load_csv_with_encoding, print_dataset_info, suggest_target_columns, get_latest_preprocessed_data
...
def run_training_pipeline(data_path, target_columns, config_path=None):
    ...
    print("Starting Stacking Ensemble Model Training Pipeline")
...
def main():
    parser = argparse.ArgumentParser(description='Stacking Ensemble Model with LLM Integration')
    ...
    if args.mode == 'optuna':
        ...
        # data 인자가 없으면 자동으로 최신 전처리 데이터 사용
        if not args.data:
            print("No data path specified. Searching for latest preprocessed data...")
            args.data = get_latest_preprocessed_data("./dataset")
            if not args.data:
                print("Error: No preprocessed data found. Please run preprocessing first:")
                print("  python main.py --mode preprocessing")
                return
            print(f"Using latest preprocessed data: {args.data}")
